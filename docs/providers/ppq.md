...
| Speech-to-Text | ❌ | Not available via profile |
| Text-to-Speech | ❌ | Not available via profile |

> PayPerQ also serves OpenAI-compatible embedding, speech-to-text, and
> text-to-speech endpoints under `https://api.ppq.ai/v1`. They aren't exposed
> through the profile yet because the profile mechanism currently only applies
> to `create_language()`; once it is extended to the other capabilities, PPQ's
> embedding/STT/TTS will be added the same way.
