"""Tests for OpenAI speech-to-text model implementation."""
...
def model():
    ...
    return OpenAISpeechToTextModel(model_name="whisper-1")


class TestOpenAISpeechToTextModel:
    """Test suite for OpenAI speech-to-text model."""

    async def test_initialization(self):
        """Test model initialization with valid config."""
        model = OpenAISpeechToTextModel(model_name="whisper-1")
    ...
    async def test_initialization_with_config(self):
        ...
        model = OpenAISpeechToTextModel(model_name="whisper-1", config=config)
        assert model.model_name == "whisper-1"
    ...
    async def test_initialization_invalid_config(self):
        ...
        with pytest.raises(
            ValueError,
            match="model_name must be specified for OpenAI speech-to-text model",
        ):
            OpenAISpeechToTextModel(model_name="")
    ...
    async def test_validate_config_failure_invalid_response_format(self):
        ...
        with pytest.raises(
            ValueError,
            match="response_format must be one of: json, text, srt, verbose_json, vtt",
        ):
            model.validate_config()
    ...
    async def test_client_lazy_initialization(self, model):
        ...
        with patch(
            "esperanto.providers.speech_to_text.openai.AsyncOpenAI",
    ...
    async def test_transcribe_with_config(self, model, mock_client, mock_response):
        """Test audio transcription with config values."""
        config = {
            "language": "en",
            "prompt": "Transcribe this",
            "response_format": "json",
    ...
    async def test_transcribe_with_kwargs(self, model, mock_client, mock_response):
        """Test audio transcription with additional kwargs."""
        model.client = mock_client
        audio_file = "test.mp3"

        with patch("builtins.open", mock_open()) as mock_file:
            result = await model.transcribe(
                audio_file, timestamp_granularities=["word"]
            )
